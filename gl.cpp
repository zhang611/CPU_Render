...
Matrix modelMatrix(float rotation_angle, Vec3f axis) {
    ...
    //             -y,x, 0,0,
    //             0,0,0,1;
    crossn[0][1] = -z, crossn[0][2] = y, crossn[1][0] = z, crossn[1][2] = -x,
    crossn[2][0] = -y, crossn[2][1] = x, crossn[3][3] = 1;

    Matrix rotate = Matrix::identity(4) * cosAlpha + (1 - cosAlpha) * nnT +
                    sinAlpha * crossn;
    rotate[3][3] = 1;
    return rotate;
}
...
Matrix lookAt(Vec3f eyePos, Vec3f& targetPos, Vec3f& up) {
    Vec3f z = (eyePos - targetPos).normalize();  // look at -z
    Vec3f x = cross(up, z).normalize();
    Vec3f y = cross(z, x).normalize();

    Matrix invR = Matrix::identity(4);
...
Matrix projection(float eye_fov, float aspect_ratio, float zNear, float zFar) {
    ...
    Matrix orth = Matrix::identity(4), orthTranslate = Matrix::identity(4),
           orthScale, pers2orth;
    ...
    //                 0, 0, n+f, -n*f,
    //                 0, 0, 1, 0;
    pers2orth[0][0] = n, pers2orth[1][1] = n, pers2orth[2][2] = n + f,
    pers2orth[2][3] = -n * f, pers2orth[3][2] = 1;
    // orthScale <<    2/(r - l), 0, 0, 0,
    //                 0, 2/(t - b), 0, 0,
    //                 0, 0, 2/(n - f), 0,
    //                 0, 0, 0,         1;
    orthScale[0][0] = 2 / (r - l), orthScale[1][1] = 2 / (t - b),
    orthScale[2][2] = 2 / (n - f), orthScale[3][3] = 1;
    // orthTranslate <<  1, 0, 0, -(r+l)/2,
    //                     0, 1, 0, -(t+b)/2,
    //                     0, 0, 1, -(n+f)/2,
    //                     0, 0, 0, 1;
    orthTranslate[0][3] = -(r + l) / 2, orthTranslate[1][3] = -(t + b) / 2,
    orthTranslate[2][3] = -(n + f) / 2;

    orth = orthTranslate * orthScale;
    proj = orth * pers2orth;
    return proj;
}
...
/*
    ...
    [-1, 1] -> [0, 2] -> [0,1] -> [0, w] -> [x, x+w]
    [-1, 1] -> [0, 2] -> [0,1] -> [0, h] -> [y, y+h]
    [-1, 1] -> [0, 2] -> [0,1] -> [0, d] -> [0, d]
*/
