...
//         return viewportM * projM * lookatM * modelM * gl_Vertex; // transform it to screen coordinates
//     }

//     virtual bool fragment(Vec3f barycentric, TGAColor &color) {
//         float intensity = varying_intensity * barycentric;   // interpolate intensity for the current pixel
...
// void drawmodel(TGAImage &image, Model *model, IShader & shader, float *zbuffer)
// {
...
//     }
// }
void drawmodel(TGAImage &image, Model *model, Matrix & modelM, Matrix & viewM, Matrix & projM, Matrix & viewportM, float *zbuffer)
{
...
int main(int argc, char **argv)
{
    ...
    */
    ...
    while (key != 27)
    {
        for (int i = width * height - 1; i >= 0; i--) zbuffer[i] = std::numeric_limits<float>::min();
        image.clear();
